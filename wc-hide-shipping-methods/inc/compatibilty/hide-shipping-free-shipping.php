 ...
 */
if ( ! class_exists( 'Wwp_Hide_Shipping_Methods' ) ) {
	class Wwp_Hide_Shipping_Methods {
		...
		public function add_settings( $settings ) {
			$settings[] = array(
				'title' => __( 'Shipping Method Visibility', 'woocommerce-wholesale-pricing' ),
				'type'  => 'title',
				'id'    => 'wc_hide_shipping',
			);

			$settings[] = array(
				'title'    => __( 'Free Shipping: ', 'woocommerce-wholesale-pricing' ),
				'desc'     => '',
				'id'       => 'wc_hide_shipping_options',
				'type'     => 'radio',
				'desc_tip' => true,
				'options'  => array(
					'hide_all'          => __( 'Show "Free Shipping" only (if available). Hide all the other methods', 'woocommerce-wholesale-pricing' ),
					'hide_except_local' => __( 'Show "Free Shipping" and "Local Pickup" only (if available). Hide all the other methods.', 'woocommerce-wholesale-pricing' ),
				),
			);
			...
			// ------------------------------------------------------------------
			$settings[] = array(
				'title'             => __( 'Additional methods to display alongside the above setting', 'woocommerce-wholesale-pricing' ),
				'desc'              => __( 'Select any additional shipping methods you want to display alongside free shipping (and local pickup, if applicable).', 'woocommerce-wholesale-pricing' ),
			...
			// ------------------------------------------------------------------
			$settings[] = array(
				'title' => __( 'Support & Other Plugins', 'woocommerce-wholesale-pricing' ),
				'type'  => 'title',
		...
		public function get_available_shipping_methods() {
			...
			foreach ( $default_methods as $method ) {
				...
				$available[ $key ] = $title . ' (' . __( 'Default Zone', 'woocommerce-wholesale-pricing' ) . ')';
			}
		...
		private function apply_shipping_method_filters() {
			...
			if ( 'hide_all' === $option ) {
				add_filter( 'woocommerce_package_rates', array( $this, 'hide_shipping_when_free_is_available' ), 99, 2 );
			} elseif ( 'hide_except_local' === $option ) {
				add_filter( 'woocommerce_package_rates', array( $this, 'hide_shipping_when_free_is_available_keep_local' ), 99, 2 );
			}
		}
		...
		public function hide_shipping_when_free_is_available( $rates ) {
			...
			$additional = (array) get_option( 'wc_hide_shipping_additional_methods', array() );
		...
		public function plugin_action_links( $links ) {
			$settings_link = '<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=shipping&section=options' ) ) . '">' . __( 'Settings', 'woocommerce-wholesale-pricing' ) . '</a>';
